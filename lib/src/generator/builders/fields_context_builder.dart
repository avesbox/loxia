  ...
  Iterable<Method> _buildJoinColumnGetters(EntityGenerationContext context) {
    return context.owningJoinColumns
      ...
      return Method((m) => m
        ...
        ..body = refer('field').call([literalString(relation.joinColumn!.name)],
            {}, [refer(typeWithNull)]).code
        ..lambda = true);
    });
  }

  Iterable<Method> _buildOwningRelationGetters(EntityGenerationContext context) {
      ...
      return Method((m) => m
        ...
        ..body = Block.of([
          declareFinal('targetRelation')
              .assign(refer(descriptorRef)
                  .property('relations')
                  .property('firstWhere')
                  .call([
                Method((m) => m
                  ..requiredParameters.add(Parameter((p) => p..name = 'r'))
                  ..body = refer('r')
                      .property('fieldName')
          ...
          declareFinal('joinColumn')
              .assign(refer('targetRelation').property('joinColumn').nullChecked)
              .statement,
          declareFinal('alias')
              .assign(refer('ensureRelationJoin').call([], {
                'relationName': literalString(relation.fieldName),
                'targetTableName':
                    refer(descriptorRef).property('qualifiedTableName'),
                'localColumn':
                    refer('joinColumn').property('referencedColumnName'),
