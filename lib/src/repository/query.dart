...
class QueryField<T> {
	...
	WhereExpression _compareWith<S>(QueryField<S> other, String operator) =>
		_ColumnComparisonPredicate(_alias, _column, other._alias, other._column, operator);
}
...
class SelectProjection<P> {
	const SelectProjection({required this.sql, required this.mapper, this.aggregator});
...
abstract class SelectOptions<T, P> {
	...
	void collect(QueryFieldsContext<T> context, List<SelectField> out, {String? path});
	...
	SelectProjection<P> compile(QueryFieldsContext<T> context) {
		if (!hasSelections) {
			throw StateError('$runtimeType does not have any selections. Enable at least one field.');
		}
